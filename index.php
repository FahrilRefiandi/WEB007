<?php
    require_once('config/config.php');
?>
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Home | Kasipaham</title>
    <link href="./assets/bootstrap-5.3.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-9ndCyUaIbzAi2FUVXJi0CjmCapSmO7SnpJef0486qhLnuZ2cdeRhO02iuK6FUUVM" crossorigin="anonymous" />
    <link rel="stylesheet" href="./assets/main.css">
  </head>
  <body>

    <!-- navbar -->
   <?php include($header) ?>
   
    <!-- navbar -->
    <?php
    $allDataUser=getAll("SELECT * FROM users");
    $firstDataUser=getFirst("SELECT * FROM users");
    
    
    foreach($allDataUser as $item){
      echo $item['username']."<br>";
    }
    print("<br><br>");
    print_r($firstDataUser['username']);
    ?>
    <!-- content -->
    <h1>CUMA TESTING,NGGK HARUS DIPAKE</h1>
    <!-- content -->
    
    <!-- footer -->
   <?php include($footer) ?>
    <!-- footer -->

    <script src="./assets/bootstrap-5.3.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-geWF76RCwLtnZ8qwWowPQNguL3RmwHVBC9FhGdlKrxdiJJigb/j/68SIy3Te4Bkz" crossorigin="anonymous"></script>
  </body>
</html>
